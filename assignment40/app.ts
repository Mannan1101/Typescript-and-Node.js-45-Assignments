...
//calling line includes a value for the number of tracks, add that value to the album’s Object. Make
// at least one new function call that includes the number of tracks on an album.


function makeAalbum (artist: string, title: string): { artist: string; title: string} {
    const dictionaries = {
...
let album = makeAalbum("ali", "light")
console.log(album)

album = makeAalbum("bilal", "red wave")
console.log(album)

album = makeAalbum("hamza", "seebreez")
    console.log(album)